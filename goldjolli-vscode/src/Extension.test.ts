...
const { mockNotifyApiKeySaveError, mockNotifyAzureConfigSaveError } = vi.hoisted(() => ({
	mockNotifyApiKeySaveError: vi.fn(),
	mockNotifyAzureConfigSaveError: vi.fn(),
}));
...
vi.mock("./views/SidebarWebviewProvider.js", () => ({
	SidebarWebviewProvider: class {
		...
		notifyAzureConfigSaveError = mockNotifyAzureConfigSaveError;
	},
}));
...
describe("Extension", () => {
	...
	describe("saveAzureFoundryConfig command", () => {
		// Sidebar onboarding inline Azure endpoint save path — mirrors
		// saveAnthropicApiKey above. Success is implicit (statusStore.refresh
		// flips `configured` via hasLlmCredentials); only failures post
		// azure:saveError.

		it("registers the saveAzureFoundryConfig command", () => {
			activate(makeContext());
			const handler = getRegisteredCommand("jollimemory.saveAzureFoundryConfig");
			expect(handler).toBeDefined();
		});

		it("saves azure fields + aiProvider:'azure-foundry' and refreshes statusStore on success", async () => {
			activate(makeContext());
			saveConfigScoped.mockResolvedValueOnce(undefined);
			mockStatusStore.refresh.mockClear();
			...
			expect(mockStatusStore.refresh).toHaveBeenCalled();
			expect(mockNotifyAzureConfigSaveError).not.toHaveBeenCalled();
		});

		it("strips a trailing slash from the endpoint but keeps a gateway path intact", async () => {
			activate(makeContext());
			saveConfigScoped.mockResolvedValueOnce(undefined);

			const handler = getRegisteredCommand("jollimemory.saveAzureFoundryConfig");
			await handler({
				endpoint: "https://gh-ai-gateway.azure-api.net/gh/openai/v1/chat/completions/",
				apiKey: "azure-secret",
				deployment: "gpt-4.1-mini",
				apiVersion: "preview",
			});
		...
		it("rejects when endpoint, apiKey, or deployment is missing, without saving", async () => {
			activate(makeContext());
			const handler = getRegisteredCommand("jollimemory.saveAzureFoundryConfig");
			await handler({ endpoint: "https://my-resource.openai.azure.com", apiKey: "", deployment: "gpt-4.1-mini" });

			expect(saveConfigScoped).not.toHaveBeenCalled();
			expect(mockNotifyAzureConfigSaveError).toHaveBeenCalledWith(
				"Endpoint, API key, and deployment are all required.",
			);
		});

		it("rejects a malformed endpoint URL, without saving", async () => {
			activate(makeContext());
			const handler = getRegisteredCommand("jollimemory.saveAzureFoundryConfig");
			await handler({ endpoint: "not-a-url", apiKey: "azure-secret", deployment: "gpt-4.1-mini" });

			expect(saveConfigScoped).not.toHaveBeenCalled();
			expect(mockNotifyAzureConfigSaveError).toHaveBeenCalledWith(
				"Endpoint must be a valid https URL (got: not-a-url).",
			);
		});

		it("rejects a non-https endpoint, without saving", async () => {
			activate(makeContext());
			const handler = getRegisteredCommand("jollimemory.saveAzureFoundryConfig");
			await handler({
			...
			expect(saveConfigScoped).not.toHaveBeenCalled();
			expect(mockNotifyAzureConfigSaveError).toHaveBeenCalledWith(
				"Endpoint must use https (got: http://my-resource.openai.azure.com).",
			);
		});

		it("rejects non-object input (defensive against malformed webview message) without saving", async () => {
			activate(makeContext());
			const handler = getRegisteredCommand("jollimemory.saveAzureFoundryConfig");
			await handler(42 as unknown);
		...
		it("posts azure:saveError with the error message when saveConfigScoped throws", async () => {
			activate(makeContext());
			saveConfigScoped.mockRejectedValueOnce(new Error("EROFS: read-only fs"));
			...
			expect(mockNotifyAzureConfigSaveError).toHaveBeenCalledWith("EROFS: read-only fs");
		});
	...
	describe("telemetry activation wiring", () => {
		// GoldJolli fork: Extension.ts's activate() no longer calls
		// activateExtensionTelemetry / registers onDidChangeTelemetryEnabled at
		// all (see the "GoldJolli fork: telemetry bootstrap/flush is
		// intentionally disabled" comment in Extension.ts) — telemetry is cut at
		// the wiring level here, not just inert via TELEMETRY_HARD_DISABLED
		// downstream. Skipped rather than deleted: it documents the upstream
		// wiring this fork intentionally removed.
		it.skip("passes showNotice / openExternal callbacks that route to vscode, and re-inits on telemetry toggle", async () => {
			activate(makeContext());
		...
		it("never calls activateExtensionTelemetry or reinitExtensionTelemetry (GoldJolli fork removes the wiring)", () => {
			activate(makeContext());
			expect(vi.mocked(activateExtensionTelemetry)).not.toHaveBeenCalled();
			expect(vi.mocked(reinitExtensionTelemetry)).not.toHaveBeenCalled();
		});
	});
