...
export function activate(context: vscode.ExtensionContext): void {
	...
	// GoldJolli fork: telemetry bootstrap/flush is intentionally disabled.
	...
	context.subscriptions.push(
		...
		// Inline onboarding Azure Foundry save — mirrors saveAnthropicApiKey
		// above (see its comment for the general shape). Writes
		// `aiProvider: "azure-foundry"` for the same reason that command
		// writes "anthropic": clicking this card is the user's explicit
		// provider choice, and it must outlast a later Jolli sign-in
		// (`saveAuthCredentials` in AuthConfig.ts already leaves an existing
		// `azure-foundry` choice alone).
		//
		// Endpoint validation mirrors ConfigureCommand's `coerceConfigValue`
		// for `azureEndpoint` (https-only; trailing slash stripped but the
		// rest of the path is kept intact — a gateway URL like
		// `.../openai/v1/chat/completions` needs its full path to survive so
		// `callAzureFoundry`'s pre-built-URL detection still works). Keep the
		// two in lockstep if that validation ever changes.
		vscode.commands.registerCommand(
			"jollimemory.saveAzureFoundryConfig",
			async (raw: unknown) => {
				log.info("cmd", "saveAzureFoundryConfig invoked");
				...
				const deployment = typeof args.deployment === "string" ? args.deployment.trim() : "";
				const apiVersion = typeof args.apiVersion === "string" ? args.apiVersion.trim() : "";
				if (endpointRaw.length === 0 || apiKey.length === 0 || deployment.length === 0) {
					sidebarProvider.notifyAzureConfigSaveError("Endpoint, API key, and deployment are all required.");
					return;
				}
				let parsedEndpoint: URL;
				try {
					parsedEndpoint = new URL(endpointRaw);
				} catch {
					sidebarProvider.notifyAzureConfigSaveError(`Endpoint must be a valid https URL (got: ${endpointRaw}).`);
					return;
				}
				if (parsedEndpoint.protocol !== "https:") {
					sidebarProvider.notifyAzureConfigSaveError(`Endpoint must use https (got: ${endpointRaw}).`);
					return;
				}
				...
				try {
					await saveConfigScoped(
						{
							...
							azureApiVersion: apiVersion.length > 0 ? apiVersion : undefined,
					...
					const message =
						err instanceof Error ? err.message : "Failed to save the Azure configuration.";
					log.error("cmd", `saveAzureFoundryConfig failed: ${message}`);
					sidebarProvider.notifyAzureConfigSaveError(message);
				}
			},
		),
