...
function buildFullStatusItems(
	...
	//   - the user has explicitly chosen a non-Anthropic provider (`jolli`,
	//     `local-agent`, `azure-foundry`) so this warning would be irrelevant, or
...
function pushProviderItem(
	...
	switch (source) {
		...
		case "azure-foundry":
			...
			icon = ICON_OK;
			tooltip =
				"AI summaries are generated via your Azure endpoint + deployment + API key.";
			break;
		default: {
			...
			} else if (config?.aiProvider === "azure-foundry") {
				tooltip =
					"Provider is set to Azure Foundry but endpoint, API key, or deployment is missing. Open Settings to configure them.";
			} else if (config?.aiProvider === "anthropic") {
				tooltip =
					"Provider is set to Anthropic but no API key is configured. Open Settings to add one.";
			} else {
				tooltip =
					"No AI provider is configured. Pick one (Anthropic, Jolli, local-agent, or Azure Foundry) in Settings.";
			}
			break;
		}
	}
