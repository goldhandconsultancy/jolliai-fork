...
/**
 * GoldJolli fork: `TELEMETRY_HARD_DISABLED` makes every export here an
 * unconditional no-op, so the upstream tests below that assert an actual
 * bootstrap/track/flush call are skipped rather than deleted — see the
 * matching note in the CLI's Telemetry.test.ts.
 */
...
describe("activateExtensionTelemetry", () => {
	itUnlessHardDisabled("emits client_activated once on activation (JOLLI-1963)", async () => {
		const { deps } = makeDeps();
		await activateExtensionTelemetry("/repo", deps);
		expect(track).toHaveBeenCalledWith("client_activated");
	});

	itUnlessHardDisabled("bootstraps with the platform opt-out signal", async () => {
		const { deps } = makeDeps({ platformDisabled: true });
		await activateExtensionTelemetry("/repo", deps);
		expect(bootstrapTelemetry).toHaveBeenCalledWith({ cwd: "/repo", platformDisabled: true });
	});

	itUnlessHardDisabled("shows the notice once and records it when enabled", async () => {
	...
	itUnlessHardDisabled("opens the docs URL when the user clicks Learn more", async () => {
		shouldShowTelemetryNotice.mockReturnValue(true);
	...
	itUnlessHardDisabled("turns telemetry off when the user clicks Turn off", async () => {
		shouldShowTelemetryNotice.mockReturnValue(true);
		const { deps, openExternal } = makeDeps();
...
describe("flushExtensionTelemetry", () => {
	itUnlessHardDisabled("delegates to flushTelemetryNow, threading the platform opt-out signal", () => {
		flushExtensionTelemetry("/repo", true);
		expect(flushTelemetryNow).toHaveBeenCalledWith("/repo", { platformDisabled: true });
	});

	itUnlessHardDisabled("passes platformDisabled=false when the host telemetry is enabled", () => {
...
describe("reinitExtensionTelemetry", () => {
	itUnlessHardDisabled("re-bootstraps with the new platform signal (no notice)", async () => {
...
describe("TELEMETRY_HARD_DISABLED (GoldJolli fork kill switch)", () => {
	it("is on and makes every export here a no-op", async () => {
		expect(TELEMETRY_HARD_DISABLED).toBe(true);
		const { deps } = makeDeps();
		await activateExtensionTelemetry("/repo", deps);
		flushExtensionTelemetry("/repo", false);
		await reinitExtensionTelemetry("/repo", false);
		expect(bootstrapTelemetry).not.toHaveBeenCalled();
