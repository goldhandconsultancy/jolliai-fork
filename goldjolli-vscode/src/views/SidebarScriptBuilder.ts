...
export function buildSidebarScript(): string {
  ...
  const onboardingAzureBtn = document.getElementById('onboarding-azure-btn');
  ...
  // Azure endpoint entry panel — same pattern as apikey-panel above (a
  // sibling of onboarding-panel, mutually exclusive with it), but with 4
  // fields instead of 1.
  ...
  const azureApiKeyInput = document.getElementById('azure-apikey-input');
  const azureDeploymentInput = document.getElementById('azure-deployment-input');
  const azureApiVersionInput = document.getElementById('azure-apiversion-input');
  const azureError = document.getElementById('azure-error');
  const azureSaveBtn = document.getElementById('azure-save-btn');
  const azureBackBtn = document.getElementById('azure-back-btn');
  ...
  onboardingAzureBtn.addEventListener('click', function() {
    showAzurePanel();
  });

  // ---- API key entry panel ----
  // showApikeyPanel / showOnboardingPanel toggle exclusively between the
  // configured===false views. Only one of {onboarding-panel, apikey-panel,
  // azure-panel} is visible at a time. All are hidden simultaneously by
  // applyConfigured(true) when the user finishes configuring (typed key
  // saved, Azure config saved, or signed in).
  function showApikeyPanel() {
    onboardingPanel.classList.add('hidden');
    azurePanel.classList.add('hidden');
  ...
  // ---- Azure endpoint entry panel ----
  // Same shape as the API key panel above, but with 4 fields. Endpoint,
  // API key, and deployment are required; API version is optional (the CLI
  // defaults it for the classic resource-origin URL shape and ignores it
  // for a pre-built gateway URL unless the user sets one explicitly).
  function showAzurePanel() {
    onboardingPanel.classList.add('hidden');
    apikeyPanel.classList.add('hidden');
    azurePanel.classList.remove('hidden');
    azureError.classList.add('hidden');
    azureError.textContent = '';
    azureEndpointInput.value = '';
    azureApiKeyInput.value = '';
    azureDeploymentInput.value = '';
    azureApiVersionInput.value = '';
    azureSaveBtn.disabled = true;
    azureSaveBtn.textContent = 'Save';
    setTimeout(function() { azureEndpointInput.focus(); }, 0);
  }
  function showOnboardingPanelFromAzure() {
    azurePanel.classList.add('hidden');
    onboardingPanel.classList.remove('hidden');
  }
  azureBackBtn.addEventListener('click', showOnboardingPanelFromAzure);
  function updateAzureSaveEnabled() {
    azureSaveBtn.disabled = azureEndpointInput.value.trim().length === 0
      || azureApiKeyInput.value.trim().length === 0
      || azureDeploymentInput.value.trim().length === 0;
  }
  function clearAzureError() {
    if (!azureError.classList.contains('hidden')) {
      azureError.classList.add('hidden');
      azureError.textContent = '';
    }
  }
  [azureEndpointInput, azureApiKeyInput, azureDeploymentInput, azureApiVersionInput].forEach(function(input) {
    input.addEventListener('input', function() {
      updateAzureSaveEnabled();
      clearAzureError();
    });
    input.addEventListener('keydown', function(e) {
      if (e.key === 'Enter' && !azureSaveBtn.disabled) {
        e.preventDefault();
        submitAzure();
      }
    });
  });
  azureSaveBtn.addEventListener('click', submitAzure);
  function submitAzure() {
    ...
    const apiVersion = azureApiVersionInput.value.trim();
    if (endpoint.length === 0 || apiKey.length === 0 || deployment.length === 0) return;
    azureSaveBtn.disabled = true;
    azureSaveBtn.textContent = 'Saving…';
    azureError.classList.add('hidden');
    azureError.textContent = '';
    // Successful save flips configured via statusStore.onChange → the panel
    // is hidden by applyConfigured(true). Failure comes back as an
    // 'azure:saveError' message handled below.
    vscode.postMessage({
      type: 'command',
      command: 'jollimemory.saveAzureFoundryConfig',
      args: [{ endpoint: endpoint, apiKey: apiKey, deployment: deployment, apiVersion: apiVersion }],
    });
  }
  ...
  function handleMessage(msg) {
    ...
    switch (msg.type) {
      ...
      case 'apikey:saveError':
        ...
        break;
      case 'azure:saveError':
        // Mirrors apikey:saveError above, for the azure-panel.
        if (!azurePanel.classList.contains('hidden')) {
          updateAzureSaveEnabled();
          azureSaveBtn.textContent = 'Save';
          azureError.textContent = typeof msg.message === 'string' && msg.message.length > 0
            ? msg.message
            : 'Failed to save the Azure configuration.';
          azureError.classList.remove('hidden');
        }
        break;
      case 'worker:busy': {
        const next = !!msg.busy;
  ...
  function applyConfigured(configured) {
    ...
    if (!configured) {
      ...
      // user signed out). The apikey-panel / azure-panel are sub-views of
      // onboarding, not a stable state on their own, so we always reset
      // back to cards.
      onboardingPanel.classList.remove('hidden');
      apikeyPanel.classList.add('hidden');
      azurePanel.classList.add('hidden');
    ...
    onboardingPanel.classList.add('hidden');
    // Hide apikey-panel / azure-panel here too — when the user successfully
    // saves a key or an Azure config, statusStore re-derives configured=true
    // and we land here. This is the single hand-off that retires whichever
    // input view was active.
    apikeyPanel.classList.add('hidden');
    azurePanel.classList.add('hidden');
