...
describe("SidebarWebviewProvider edge paths", () => {
	describe("telemetry flush on the conversations tick", () => {
		// GoldJolli fork: the 60s conversations tick in SidebarWebviewProvider.ts
		// no longer calls flushExtensionTelemetry at all (removed alongside the
		// rest of the telemetry wiring — see Extension.ts's "GoldJolli fork:
		// telemetry bootstrap/flush is intentionally disabled" comment).
		// Skipped rather than deleted: documents the upstream behavior this
		// fork intentionally removed. See "never flushes telemetry" below for
		// the current behavior.
		it.skip("flushes buffered telemetry with the workspace cwd and inverted platform consent", async () => {
			vi.useFakeTimers();
			try {
		...
		it("never flushes telemetry on the tick, even with a workspace folder open (GoldJolli fork)", async () => {
			vi.useFakeTimers();
			try {
				mockWorkspaceFolders.push({ uri: { fsPath: "/proj" } });
				const listWithDiagnostics = vi.fn().mockResolvedValue({ items: [], failedSources: [] });
