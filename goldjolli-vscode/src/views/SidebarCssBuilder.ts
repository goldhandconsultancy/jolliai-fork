...
export function buildSidebarCss(): string {
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* ── API key / Azure endpoint entry panels ────────────────────────
     Reuses .ob-header / .ob-btn from the onboarding panel. The label +
     input pair(s) sit between the header and the Save/Back buttons. The
     inline error span is hidden via .hidden by default and surfaces
     only after the host posts an apikey:saveError / azure:saveError back.
     The azure-panel (4 stacked fields) reuses these same classes. */
  .apikey-label {
    display: block;
    font-size: 12px;
    font-weight: 600;
