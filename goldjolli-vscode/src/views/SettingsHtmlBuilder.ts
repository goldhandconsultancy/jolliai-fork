 ...
 * SettingsHtmlBuilder
 *
 * Assembles the complete HTML document for the Settings webview as a 6-tab
 ...
 *   6. Advanced      — extra config fields not used in common flows
 */
...
<body>
  <div class="settings-page">
    ...
    <nav class="tab-nav" role="tablist">
      ...
      <button type="button" class="tab-button" role="tab" data-tab="advanced">Advanced</button>
    </nav>
    ...
    <section class="tab-panel hidden" data-panel="summary" role="tabpanel">
      <div class="settings-row">
        ...
        <select id="aiProvider">
          ...
          <option value="azure-foundry">Azure Foundry / Azure OpenAI</option>
      ...
      <div class="card-panel hidden" data-card="azure-foundry">
        <p class="section-hint">Use your own Azure AI Foundry / Azure OpenAI deployment endpoint and key — a bare resource origin, or a full gateway route (e.g. an APIM front door) ending in /chat/completions.</p>
        <div class="settings-row">
          <label class="settings-label" for="azureEndpoint">
            Endpoint
            <span class="hint">https://your-resource.openai.azure.com, or a full gateway URL like https://your-gateway.azure-api.net/path/openai/v1/chat/completions</span>
          </label>
          <div class="input-col">
            <input type="text" id="azureEndpoint" placeholder="https://...openai.azure.com" autocomplete="off" spellcheck="false" />
            <div class="error-message" id="azureEndpoint-error"></div>
          </div>
        </div>
        <div class="settings-row">
          <label class="settings-label" for="azureApiKey">
            API Key
            <span class="hint">Stored in ~/.jolli/jollimemory/config.json</span>
          </label>
          <div class="input-col">
            <input type="text" id="azureApiKey" placeholder="Azure API key" autocomplete="off" spellcheck="false" />
            <div class="error-message" id="azureApiKey-error"></div>
          </div>
        </div>
        <div class="settings-row">
          <label class="settings-label" for="azureDeployment">
            Deployment
            <span class="hint">Deployment name (resource origin mode) — also sent as the "model" field when using a gateway URL</span>
          </label>
          <div class="input-col">
            <input type="text" id="azureDeployment" placeholder="gpt-4.1" autocomplete="off" spellcheck="false" />
            <div class="error-message" id="azureDeployment-error"></div>
          </div>
        </div>
        <div class="settings-row">
          <label class="settings-label" for="azureApiVersion">
            API Version
            <span class="hint">Optional override — only used for the resource-origin URL shape; leave empty for a gateway URL</span>
          </label>
          <input type="text" id="azureApiVersion" placeholder="2024-06-01" autocomplete="off" spellcheck="false" />
        </div>
      </div>
    ...
    <!-- ── Tab 6: Advanced ── -->
    <section class="tab-panel hidden" data-panel="advanced" role="tabpanel">
      <p class="section-hint">Advanced options for path overrides, URL allowlists, MCP tooling, and logging. Leave fields empty to use defaults.</p>

      <div class="toggle-row">
        <label class="settings-label" for="mcpPlatformToolsEnabled">
          Enable Jolli MCP Platform Tools
          <span class="hint">When enabled, Jolli MCP can register backend-defined platform tools from your tenant manifest.</span>
        </label>
        <label class="toggle-switch">
          <input type="checkbox" id="mcpPlatformToolsEnabled" />
          <span class="toggle-slider"></span>
        </label>
      </div>

      <div class="settings-row column">
        <label class="settings-label" for="summaryLanguage">
          AI Output Language
          <span class="hint">Language for reader-facing AI text (commit messages, summaries, recaps, e2e tests, wiki text), e.g. Dutch. Leave empty for the model's default (English). Structural markers, IDs, and code identifiers are never translated.</span>
        </label>
        <div class="input-col">
          <input type="text" id="summaryLanguage" placeholder="e.g. Dutch" spellcheck="false" />
          <div class="error-message" id="summaryLanguage-error"></div>
        </div>
      </div>

      <div class="settings-row column">
        <label class="settings-label" for="localAgentPath">
          Local Agent Path
          <span class="hint">Optional absolute path to agent binary (overrides PATH discovery), e.g. /usr/local/bin/claude.</span>
        </label>
        <div class="input-col">
          <input type="text" id="localAgentPath" placeholder="/absolute/path/to/agent" spellcheck="false" />
          <div class="error-message" id="localAgentPath-error"></div>
        </div>
      </div>

      <div class="settings-row column">
        <label class="settings-label" for="openUrlAllowedOrigins">
          Extra Allowed URL Origins
          <span class="hint">Comma-separated hosts or https origins for open-url allowlist extensions, e.g. x.ngrok-free.dev, https://preview.example.dev.</span>
        </label>
        <div class="input-col">
          <input type="text" id="openUrlAllowedOrigins" placeholder="x.ngrok-free.dev, https://preview.example.dev" spellcheck="false" />
          <div class="error-message" id="openUrlAllowedOrigins-error"></div>
        </div>
      </div>

      <div class="settings-row">
        <label class="settings-label" for="logLevel">
          Log Level
          <span class="hint">Global debug log level.</span>
        </label>
        <select id="logLevel">
      ...
      <div class="settings-row column">
        <label class="settings-label" for="logLevelOverridesJson">
          Log Level Overrides (JSON)
          <span class="hint">Optional JSON object of module-to-level mappings, e.g. {"QueueWorker":"debug","GitOps":"warn"}.</span>
        </label>
        <div class="input-col">
          <textarea id="logLevelOverridesJson" rows="5" placeholder='{"QueueWorker":"debug"}' spellcheck="false"></textarea>
          <div class="error-message" id="logLevelOverridesJson-error"></div>
        </div>
      </div>
    </section>
  </div>
