...
interface SettingsPayload {
	...
	readonly openUrlAllowedOrigins?: string;
	readonly logLevel?: "debug" | "info" | "warn" | "error" | "";
	readonly logLevelOverridesJson?: string;
...
export class SettingsWebviewPanel {
	 ...
	 */
	private resolveProvider(config: JolliMemoryConfig): "anthropic" | "jolli" | "local-agent" | "azure-foundry" {
		if (
	...
	private async handleLoadSettings(): Promise<void> {
		...
		this.fullAzureApiKey = config.azureApiKey ?? "";
		...
		const payload: SettingsPayload = {
			...
			azureApiKey: maskApiKey(config.azureApiKey),
			azureDeployment: config.azureDeployment ?? "",
			// Left blank (not defaulted to "2024-06-01") when unset: the visible
			// placeholder already hints the classic-mode default, but a literal
			// value here would get saved verbatim and wrongly append
			// ?api-version=... to a gateway URL that doesn't want one.
			azureApiVersion: config.azureApiVersion ?? "",
			mcpPlatformToolsEnabled: config.mcpPlatformToolsEnabled === true,
			summaryLanguage: config.summaryLanguage ?? "",
			openUrlAllowedOrigins: config.openUrlAllowedOrigins
				? config.openUrlAllowedOrigins.join(", ")
				: "",
			logLevel: config.logLevel ?? "",
			logLevelOverridesJson: config.logLevelOverrides
				? JSON.stringify(config.logLevelOverrides, null, 2)
				: "",
	...
	private async handleApplySettings(
		...
		const resolvedJolliApiKey =
			...
			// `?? ""` tolerates a payload that omits these fields, same as
			// compileExcludeFolders/openUrlAllowedOrigins/localAgentPath below —
			// the live webview always sends them, but a direct postMessage
			// (tests, or a stale cached bundle from before Azure support existed)
			// shouldn't crash the save.
			const resolvedAzureApiKey = settings.azureApiKey === maskApiKey(this.fullAzureApiKey)
				? this.fullAzureApiKey
		...
		const openUrlAllowedOrigins = (settings.openUrlAllowedOrigins ?? "")
			.split(",")
			.map((p) => p.trim())
			.filter((p) => p.length > 0);

		let logLevelOverrides: Record<string, "debug" | "info" | "warn" | "error"> | undefined;
		if ((settings.logLevelOverridesJson ?? "").trim().length > 0) {
			let parsed: unknown;
			try {
				parsed = JSON.parse(settings.logLevelOverridesJson ?? "");
			} catch {
				this.postError("Log Level Overrides must be valid JSON");
				return;
			}
			if (
				...
				this.postError("Log Level Overrides must be a JSON object");
				return;
			}
			...
			for (const [k, v] of Object.entries(parsed as Record<string, unknown>)) {
				if (typeof v !== "string" || !allowed.has(v)) {
					this.postError(
						"Log Level Overrides values must be one of: debug, info, warn, error",
					);
					return;
				}
				out[k] = v as "debug" | "info" | "warn" | "error";
			}
			logLevelOverrides = out;
		}
		...
		const update: Partial<JolliMemoryConfig> = {
			...
			localAgentPath:
				(settings.localAgentPath ?? "").trim().length > 0
					? (settings.localAgentPath ?? "").trim()
					: undefined,
			jolliApiKey:
				resolvedJolliApiKey.length > 0 ? resolvedJolliApiKey : undefined,
			azureEndpoint:
				(settings.azureEndpoint ?? "").trim().length > 0 ? (settings.azureEndpoint ?? "").trim() : undefined,
			azureApiKey:
				resolvedAzureApiKey.trim().length > 0 ? resolvedAzureApiKey.trim() : undefined,
			azureDeployment:
				(settings.azureDeployment ?? "").trim().length > 0 ? (settings.azureDeployment ?? "").trim() : undefined,
			azureApiVersion:
				(settings.azureApiVersion ?? "").trim().length > 0 ? (settings.azureApiVersion ?? "").trim() : undefined,
			...
			mcpPlatformToolsEnabled:
				settings.mcpPlatformToolsEnabled === true ? true : undefined,
			summaryLanguage:
				(settings.summaryLanguage ?? "").trim().length > 0 ? (settings.summaryLanguage ?? "").trim() : undefined,
			openUrlAllowedOrigins:
				openUrlAllowedOrigins.length > 0 ? openUrlAllowedOrigins : undefined,
			logLevel:
		...
		this.fullAzureApiKey = resolvedAzureApiKey;
