...
export type SidebarInboundMsg =
	...
	| {
			/**
			 * Posted only on the failure path of the inline onboarding Azure
			 * Foundry save (jollimemory.saveAzureFoundryConfig). Mirrors
			 * `apikey:saveError` above — see its comment for the success-path
			 * shape, which is identical.
			 */
			readonly type: "azure:saveError";
			readonly message: string;
	  }
	| {
			/**
