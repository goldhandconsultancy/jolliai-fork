...
describe("SettingsWebviewPanel", () => {
	...
	describe("handleApplySettings", () => {
		...
		describe("logLevelOverridesJson validation", () => {
			async function applyWithOverridesJson(overridesJson: string) {
				const dispatch = await setupWithLoadedConfig();
				dispatch({
					command: "applySettings",
					maskedApiKey: "",
					...
					settings: {
						...
						geminiEnabled: true,
						excludePatterns: "",
						logLevelOverridesJson: overridesJson,
					},
				});
				await flushPromises();
			}
			...
			it("rejects malformed JSON, without saving", async () => {
				await applyWithOverridesJson("{not json");
				expect(mockSaveConfigScoped).not.toHaveBeenCalled();
				expect(postMessage).toHaveBeenCalledWith(
					expect.objectContaining({
						command: "settingsError",
						message: "Log Level Overrides must be valid JSON",
					}),
				);
			});

			it("rejects a JSON array (must be an object), without saving", async () => {
				await applyWithOverridesJson("[]");
				expect(mockSaveConfigScoped).not.toHaveBeenCalled();
				expect(postMessage).toHaveBeenCalledWith(
					expect.objectContaining({
						command: "settingsError",
						message: "Log Level Overrides must be a JSON object",
					}),
				);
			});

			it("rejects an override value outside debug/info/warn/error, without saving", async () => {
				await applyWithOverridesJson('{"QueueWorker":"verbose"}');
				expect(mockSaveConfigScoped).not.toHaveBeenCalled();
				expect(postMessage).toHaveBeenCalledWith(
					expect.objectContaining({
						command: "settingsError",
						message: "Log Level Overrides values must be one of: debug, info, warn, error",
					}),
				);
			});

			it("persists a valid overrides object", async () => {
				await applyWithOverridesJson('{"QueueWorker":"debug","GitOps":"warn"}');
				expect(mockSaveConfigScoped).toHaveBeenCalledWith(
					expect.objectContaining({
						logLevelOverrides: { QueueWorker: "debug", GitOps: "warn" },
					}),
					expect.any(String),
				);
			});
	...
	describe("aiProvider + auth state", () => {
		...
		it("applySettings persists a trimmed summaryLanguage", async () => {
			mockLoadConfigFromDir.mockResolvedValue({
				apiKey: undefined,
				model: "sonnet",
				maxTokens: null,
			...
			expect(mockSaveConfigScoped).toHaveBeenCalledWith(
				expect.objectContaining({ summaryLanguage: "Dutch" }),
				expect.any(String),
			);
		});

		it("applySettings omits summaryLanguage (undefined) when the field is left blank", async () => {
			mockLoadConfigFromDir.mockResolvedValue({
				apiKey: undefined,
				model: "sonnet",
				maxTokens: null,
			...
			expect(mockSaveConfigScoped).toHaveBeenCalledWith(
				expect.objectContaining({ summaryLanguage: undefined }),
				expect.any(String),
			);
		});
	});
