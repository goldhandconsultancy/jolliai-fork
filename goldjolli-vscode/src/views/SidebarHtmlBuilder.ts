 ...
 *     Shown when `state.configured === false` (no usable LLM credential at
 *     all — see `hasLlmCredentials`). In that mode the tab bar and tab content
 *     are hidden; the onboarding panel takes the entire viewport. Three
 *     options: Anthropic API key (recommended primary), Sign in to Jolli, and
 *     a third "Use your own Azure endpoint" card for a self-hosted Azure AI
 *     Foundry / Azure OpenAI deployment (or a gateway in front of one).
 ...
 *   - An Azure endpoint entry panel sibling, hidden by default. Shown when the
 *     user clicks "Configure Azure Endpoint" — same pattern as the API key
 *     panel, but with 4 fields (endpoint, key, deployment, optional api
 *     version). Mirrors the CLI's own `azureEndpoint`/`azureApiKey`/
 *     `azureDeployment`/`azureApiVersion` config keys 1:1.
...
<body>
  <div class="sidebar-root" id="sidebar-root">
    ...
    <div class="onboarding-panel hidden" id="onboarding-panel" role="region" aria-label="Get started with Jolli Memory">
      ...
      <section class="ob-card">
        <div class="ob-card-row">
          <i class="codicon codicon-server-environment ob-card-icon" aria-hidden="true"></i>
          <div class="ob-card-text">
            <h3 class="ob-card-title">Use your own Azure endpoint</h3>
            <p class="ob-card-desc">Connect an Azure AI Foundry / Azure OpenAI deployment (or a gateway in front of one). Memories are stored locally only; nothing goes through Jolli or Anthropic.</p>
          </div>
        </div>
      </section>
      <button type="button" id="onboarding-azure-btn" class="ob-btn ob-btn--secondary">Configure Azure Endpoint</button>
    </div>
    ...
    <div class="azure-panel hidden" id="azure-panel" role="region" aria-label="Configure Azure endpoint">
      <header class="ob-header">
        <div class="ob-title-row">
          <i class="codicon codicon-server-environment ob-title-icon" aria-hidden="true"></i>
          <h2 class="ob-title">Configure your Azure endpoint</h2>
        </div>
        <p class="ob-subtitle">A resource origin (https://your-resource.openai.azure.com) or a full gateway route ending in /chat/completions. Stored locally only.</p>
      </header>
      <label class="apikey-label" for="azure-endpoint-input">Endpoint</label>
      <input type="text" id="azure-endpoint-input" class="apikey-input" autocomplete="off" spellcheck="false" placeholder="https://...openai.azure.com" />
      <label class="apikey-label" for="azure-apikey-input">API key</label>
      <input type="password" id="azure-apikey-input" class="apikey-input" autocomplete="off" spellcheck="false" placeholder="Azure API key" />
      <label class="apikey-label" for="azure-deployment-input">Deployment</label>
      <input type="text" id="azure-deployment-input" class="apikey-input" autocomplete="off" spellcheck="false" placeholder="gpt-4.1" />
      <label class="apikey-label" for="azure-apiversion-input">API version <span class="ob-optional">(optional)</span></label>
      <input type="text" id="azure-apiversion-input" class="apikey-input" autocomplete="off" spellcheck="false" placeholder="2024-06-01" />
      <p class="apikey-error hidden" id="azure-error" role="alert"></p>
      <button type="button" id="azure-save-btn" class="ob-btn ob-btn--primary" disabled>Save</button>
      <button type="button" id="azure-back-btn" class="ob-btn ob-btn--secondary">Back</button>
    </div>
